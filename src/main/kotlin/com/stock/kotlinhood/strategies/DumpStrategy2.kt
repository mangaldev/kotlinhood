...
class DumpStrategy2 {
    companion object {
        ...
        val STOP_LOSS_RUNNING_UP_MARGIN = 0.7
        val STOP_LOSS_DOWN_MARGIN = 0.50
        var positiveTrend = 0
        var lastQuote = 0.0
        var buyCounter = 0
        var currentDateTime: LocalDateTime = LocalDateTime.MAX
        val stocks = listOf<String>("GOOG", "GOOGL", "FB", "AMZN", "HON","ANET","BAC")
        var totalProfit = 0.0
        @JvmStatic
        fun main(args: Array<String>) {
            stocks.forEach { stock ->
                ...
                val fromJson = Gson().fromJson<LinkedTreeMap<String, Any>>(
                    File("/Users/mdev/Google Drive/TextWrangler/robinhood/10Apr/$stock-2weeks-5Min.json").readText(Charset.defaultCharset()),
                    Map::class.java
                )
...
//            targetStopLoss(closingPrice)
        ...
        fun crossedBelowMargin(lastBoughtAt: Double, closingPrice: Double): Boolean {
            ...
            assert(downMargin > 0) { print("Checking down stop loss closingPrice should be lesser than lastBoughtAt") }
...
//                    return
        ...
        private fun println(message: Any, show: Boolean = false) {
            if (show)
